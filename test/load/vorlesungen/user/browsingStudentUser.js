...
export function browsingStudentUser(user, thinkTime, ctx){
    ...
    group('Browse Web UI', () => {
        ...
        const statistikView = http.get(K6Api.lectures.getLectures(userId, semesterId), params)

        check(statistikView, {
            'Statistik View status 200': (r) => r.status === 200,
        })
        think(thinkTime);
        ...
        const quizduelView = http.get(K6Api.quizduel.getQuizduel, params)

        check(quizduelView, {
            'Quizduel View status 200': (r) => r.status === 200,
        })
        ...
        http.get(K6Api.courses.getCourseSubscribedList, params)
        ...
        http.get(K6Api.courseDeadline.getCoursesDeadlineSub, params)
        http.get(K6Api.quizduelDeadline.getQuizduellDeadlineSub, params)
        const subscriptionsList = http.get(K6Api.subscriptions.getSubscriptionList, params)

        check(subscriptionsList, {
            'subscriptions List status 403 for student': (r) => r.status === 403,
        })
        ...
        const examListView = http.get(K6Api.exam.getListUserExam(userId), params)

        check(examListView, {
            'ExamList View status 200': (r) => r.status === 200,
        })
    ...
    group('jump Back to Taskprogress', () => {
        http.get(K6Api.auth.isAuthenticated, params);
        http.get(K6Api.auth.getUser, params);
    ...
    group('jump back to Lecture View', () => {
        http.get(K6Api.auth.isAuthenticated, params);
        http.get(K6Api.auth.getUser, params);
